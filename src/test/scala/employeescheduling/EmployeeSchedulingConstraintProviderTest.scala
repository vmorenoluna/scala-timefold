...
class EmployeeSchedulingConstraintProviderTest extends AnyFlatSpec {
  ...
  "requiredSkill" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.requiredSkill(cf))
      .given(
        employee,
  ...
  "overlappingShifts" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.noOverlappingShifts(cf))
      .given(
        employee1,
        employee2,
  ...
  "atLeast10HoursBetweenConsecutiveShifts" should "work" in {
    val employee1: Employee = Employee("Amy", null, null, null, null)
    val employee2: Employee = Employee("Beth", null, null, null, null)
    constraintVerifier.verifyThat((cp, cf) => cp.atLeast10HoursBetweenTwoShifts(cf))
      .given(employee1, employee2,
        Shift("1", DAY_START_TIME, DAY_END_TIME, "Location", "Skill", employee1),
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.atLeast10HoursBetweenTwoShifts(cf))
      .given(employee1, employee2,
        Shift("1", DAY_START_TIME, DAY_END_TIME, "Location", "Skill", employee1),
        Shift("2", DAY_END_TIME.plusHours(10), DAY_START_TIME.plusDays(1), "Location 2", "Skill",
          employee1))
      .penalizes(0)
    constraintVerifier.verifyThat((cp, cf) => cp.atLeast10HoursBetweenTwoShifts(cf))
      .given(employee1, employee2,
  ...
  "unavailableEmployee" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.unavailableEmployee(cf))
      .given(employee1, employee2,
        Shift("1", DAY_START_TIME, DAY_END_TIME, "Location", "Skill", employee1))
      .penalizesBy(
        Duration.ofHours(8).toMinutes.toInt
  ...
  "undesiredDayForEmployee" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.undesiredDayForEmployee(cf))
      .given(employee1, employee2,
        Shift("1", DAY_START_TIME, DAY_END_TIME, "Location", "Skill", employee1))
      .penalizesBy(
        Duration.ofHours(8).toMinutes.toInt
  ...
  "desiredDayForEmployee" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.desiredDayForEmployee(cf))
      .given(employee1, employee2,
        Shift("1", DAY_START_TIME, DAY_END_TIME, "Location", "Skill", employee1))
      .rewardsWith(
        Duration.ofHours(8).toMinutes.toInt
  ...
  "balanceEmployeeShiftAssignments" should "work" in {
    ...
    constraintVerifier.verifyThat((cp, cf) => cp.balanceEmployeeShiftAssignments(cf))
      .given(employee1, employee2)
      .penalizesBy(0)
